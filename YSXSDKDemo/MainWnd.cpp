...
void		MainWnd::InitWindow()
{
	...
	m_EditUserNameLogin = static_cast<CEditUI*>(m_pm.FindControl(_T("usernameedit")));
	m_EditPasswordLogin = static_cast<CEditUI*>(m_pm.FindControl(_T("passwordedit")));
...
void MainWnd::Notify(TNotifyUI& msg)
{
	...
	if (msg.sType == DUI_MSGTYPE_CLICK)
	{
		...
		if (strName == _T("loginbtn"))
		{
			...
			// 页面跳转和隐藏原页面
			pMainFrame = new CLoginFrame;
			pMainFrame->Create(NULL, _T("云视讯"), UI_WNDSTYLE_DIALOG, WS_EX_WINDOWEDGE);
...
{
	apiKey = L"35C671E7221B451AB833236FA050A0CB";
	apiSecret = L"8540A8C27D884C4085E29AA39417E2DE";
	appKey = L"CDxp0JymWh0j0H2EkOat88m69cfxbcWJytLV";
	appSecret = L"Sc0kQUDPrHYcmLMIuz8hKwCnpGwdeeoD127v";
...
void MainWnd::onLoginRet(LOGINSTATUS ret)
{
	...
	case YSXSDK::LOGIN_SUCCESS:
		...
		ShowMsg(L"登录成功");
		// 页面跳转
		JumpHomepage(m_AuthService->GetCurrentUser()->UserName.c_str());
		break;
...
//     }
	...
	{
        char buf[1025];
        sprintf(buf, "会议号：%u", m_MeetingService->GetMeetingInfo()->GetMeetingNumber());
        ShowMsg(s2ws(buf));  //显示会议号

//         HWND hWnd = FindWindow(L"Notepad", NULL);
//         m_MeetingService->GetMeetingShareController()->StartAppShare(hWnd);

		//m_LayoutMeeting->SetVisible(false);
		//m_LayoutMeetingControl->SetVisible(true);  //会议控制界面
	}
...
void MainWnd::Login()
{
	...
	if (TRUE)
	{
		...
		auto mobile = m_EditUserNameLogin->GetText();
		if (mobile.IsEmpty())
		{
			ShowMsg(L"请输入手机号码");
			return;
		...
		auto password = m_EditPasswordLogin->GetText();
		if (password.IsEmpty())
		{
			ShowMsg(L"请输入密码");
			return;
...
void MainWnd::ShowMsg(const std::wstring& msg)
{
	...
	::MessageBox(NULL, msg.c_str(), L"提示", MB_OK);
}

void MainWnd::JumpHomepage(const std::wstring& msg)
{
	::MessageBox(NULL, msg.c_str(), L"欢迎您", MB_OK);
	pMainFrame = new CLoginFrame;
	pMainFrame->Create(NULL, _T("云视讯"), UI_WNDSTYLE_DIALOG, WS_EX_WINDOWEDGE);
	pMainFrame->CenterWindow();
	pMainFrame->ShowWindow();
